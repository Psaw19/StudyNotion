...
exports.changePassword = async (req, res) => {
    try {
        ...
        //validate password
        const isPasswordMatch = await bcrypt.compare(userDetails.password, oldPassword)
        if (!isPasswordMatch) {
            return res.status(401).json({
                success: false,
        ...
        const updatedUserDetails = await User.findByIdAndUpdate(req.user.id, { password: newHashedPassword }, { new: true });
